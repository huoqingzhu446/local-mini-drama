      ...
      <div class="script-editor-pane">
          ...
          <article class="ready">
            ...
            <div class="gen-controls">
              ...
              <button
                type="button"
                class="gen-button"
                :disabled="!canGen || generatingStoryboards || busy"
                @click="emitGenerate"
              >
        ...
        <div v-if="sbDraft?.shots?.length" ref="draftPanel" class="draft-panel">
          ...
          <div class="draft-footer">
            ...
            <button type="button" class="primary" :disabled="busy || applying" @click="$emit('apply-storyboards', applyMode)">
              {{ applying ? '正在应用…' : `应用 ${sbDraft.shots.length} 个分镜` }}
            </button>
            <button type="button" class="ghost" :disabled="applying" @click="$emit('discard-draft')">丢弃草稿</button>
          </div>
        </div>
      </div>
...
const emit = defineEmits(['save', 'select-version', 'generate-storyboards', 'apply-storyboards', 'discard-draft'])
