...
function generateImage(db, log) {
  ...
  return (req, res) => {
    ...
    const quality = req.body?.quality != null ? String(req.body.quality).trim() || null : null;
    try {
      const taskId = propImageGenerationService.generatePropImage(db, log, id, { model, style, quality });
